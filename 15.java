/* 
 * Sort the array and for every 'a' item from left to right. Sweep the
 * rest of the array to find 'b' and 'c' as if we were solving a twoSum problem.
...
public class Solution {
	...
	public List<List<Integer>> threeSum(int[] num) {
		...
		/*
		 * hm is a hasmap storing the sum of all possible
		 * combinations between two entries of the array.
		 * 		key: the sum
		 * 		value: the index of the two elements that add up to this sum
		 * 
		 * problems:
		 * 1) a sum can be composed of different numbers
		 * 			(we need to store all combinations that appear)
